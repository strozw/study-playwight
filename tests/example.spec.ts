...
test.describe('New Todo', () => {
  test('should allow me to add todo items', async ({ page, queries }) => {
    const input = await queries.findByPlaceholderText('What needs to be done?')
    ...
    await queries.findByText(new RegExp(TODO_ITEMS[0]))
    ...
    // await queries.findByLabelText(new RegExp(TODO_ITEMS[0]))
    // await queries.findByLabelText(new RegExp(TODO_ITEMS[1]))
  ...
  test('should append new items to the bottom of the list', async ({
    ...
    await expect(page.locator('.todo-count')).toContainText('')
